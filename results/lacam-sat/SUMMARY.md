...
2. **Centralized planning becomes intractable at extreme density.** Per-query budget was
   60 s (default LacamInferenceConfig: time_limit=60, timeouts=[1,5,10,60]; lacam_run.py
   did NOT override it). At 512, only 1/3 seeds returned a solution (2/3 failed); at 640,
   1/3 (2/3 failed), and the one solved collapsed to 0.58 (< all-active 1.06). CAVEAT: no
   stderr was saved, so the failure MODE (timeout vs crash) is unconfirmed — paper says
   "fails to solve within the 60 s budget," not "times out." Re-run failed instances with
   stderr to confirm.
